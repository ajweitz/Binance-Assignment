...
def q1(quote_asset: str = Q1_DEFAULT_QUOTE_ASSET, results: int = DEFAULT_TOTAL_RESULTS):
    ...
    loop.run_until_complete(_print_top_symbols(
        quote_asset, Q1_COMPARE_PARAMETER, results))
...
def q2(quote_asset: str = Q2_DEFAULT_QUOTE_ASSET, results: int = DEFAULT_TOTAL_RESULTS):
    ...
    loop.run_until_complete(_print_top_symbols(
        quote_asset, Q2_COMPARE_PARAMETER, results))
...
# this way, the BinanceClient object will be only created when q6 command is called
...
def q6(quote_asset: str = Q2_DEFAULT_QUOTE_ASSET, results: int = DEFAULT_TOTAL_RESULTS):
    ...
    symbols = loop.run_until_complete(client.get_top_symbols(
        quote_asset, Q2_COMPARE_PARAMETER, results))
    for s in symbols:
        gauges.append(prometheus_client.Gauge(
            f"price_spread_delta_{s}", f"Gauge of the absolute delta of the price spread for {s}"))
